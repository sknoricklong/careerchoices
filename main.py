...
def show_app():
    ...
    for index, choice in enumerate(choices):
        ...
        selected_factors = []  # Initialize selected_factors here
        ...
        if use_default_factors:
            ...
            selected_factors = all_factors  # Assign all factors to selected_factors if using defaults
        else:
            st.markdown(f"## Customize factors for Option {index + 1}: {decision_title}")
            st.markdown("Rank the factors by importance:")
            # The logic for custom selection of factors will be here

        # Now, selected_factors is always defined before this point
        for i, _ in enumerate(all_factors):
            # Now we check if the factor is in selected_factors before showing it in the selectbox
            remaining_factors = [factor for factor in all_factors if factor not in selected_factors]
            if remaining_factors:  # Only proceed if there are remaining factors
